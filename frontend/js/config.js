// Dynamic Application Configuration - Works for both localhost and PythonAnywhere
const CONFIG = (() => {
    ...
    if (isLocalhost) {
        ...
        // Extract username from hostname (e.g., laukayson.pythonanywhere.com)
    ...
    const environmentConfig = isLocalhost ? {
        ...
        features: {
            voiceInput: true,
            pronunciationCheck: false, // May have issues on PythonAnywhere
            imageGeneration: false, // Disabled due to potential resource limits
            farsiAudio: true,
            gamification: true,
            spacedRepetition: true,
            progressTracking: true,
    ...
    return {
        // API Configuration - Dynamic URLs
        BACKEND_URL: baseUrl,
        ...
        API: {
            ...
            ENDPOINTS: {
                CHAT: '/api/chat',
                CHAT_INIT_TOPIC: '/api/chat/init-topic',
                TRANSLATE: '/api/translate',
                TTS: '/api/tts',
                STT: '/api/stt',
                STT_STATUS: '/api/stt/status',
                STT_INITIALIZE: '/api/stt/initialize',
                STT_CLEANUP: '/api/stt/cleanup',
                CONFIG_HEADLESS: '/api/config/headless',
            ...
            RATE_LIMIT: {
                REQUESTS_PER_MINUTE: environmentConfig.rateLimit.requestsPerMinute,
                REQUESTS_PER_HOUR: environmentConfig.rateLimit.requestsPerHour
            }
        },
        ...
        ENVIRONMENT: {
            ...
            IS_PYTHON_ANYWHERE: isPythonAnywhere,
            BASE_URL: baseUrl,
        ...
        FEATURES: {
            VOICE_INPUT: environmentConfig.features.voiceInput,
            PRONUNCIATION_CHECK: environmentConfig.features.pronunciationCheck,
            IMAGE_GENERATION: environmentConfig.features.imageGeneration,
            FARSI_AUDIO: environmentConfig.features.farsiAudio,
            GAMIFICATION: environmentConfig.features.gamification,
            SPACED_REPETITION: environmentConfig.features.spacedRepetition,
            PROGRESS_TRACKING: environmentConfig.features.progressTracking,
            RATE_LIMITING: environmentConfig.features.rateLimiting
        },
        ...
        // Development Settings - Environment-specific
        DEV: {
            DEBUG: isLocalhost,
            MOCK_API: false,
            LOG_LEVEL: environmentConfig.logLevel
        }
    };
})();
...
CONFIG.getEnvironmentInfo = function() {
    return {
        environment: this.ENVIRONMENT.IS_LOCALHOST ? 'development' : 'production',
        platform: this.ENVIRONMENT.IS_PYTHON_ANYWHERE ? 'pythonanywhere' : 'other',
        baseUrl: this.ENVIRONMENT.BASE_URL,
